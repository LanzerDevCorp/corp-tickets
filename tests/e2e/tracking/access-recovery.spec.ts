...
test.describe("Track access recovery", () => {
  test("muestra formulario al acceder con session_expired", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: /entrar al ticket/i }),
    ).toBeVisible();
  });

  test("muestra formulario al acceder directamente sin error_code", async ({
    page,
  }) => {
  ...
  test("pre-rellena campos desde query params", async ({ page }) => {
    ...
    await expect(
      page.locator('input[value="cliente@ejemplo.com"]'),
    ).toBeVisible();
  });
  ...
  test("muestra copy de sesión expirada con session_expired", async ({
    page,
  }) => {
    await page.goto("/track/access?error_code=session_expired");
