...
class MockResend {
  emails = {
    send: async (payload: {
      ...
      console.log(
        `[MockResend] Sending email via local SMTP (Mailpit) to: ${payload.to}...`,
      );
      try {
        ...
        const info = await transporter.sendMail({
          ...
          replyTo: Array.isArray(payload.reply_to)
            ? payload.reply_to.join(", ")
            : payload.reply_to,
        });

        console.log(
          `[MockResend] Email sent successfully. MessageId: ${info.messageId}`,
        );
